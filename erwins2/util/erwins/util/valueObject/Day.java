...
public class Day implements ValueObject,Comparable<Day> {
	...
	public static Day now() {
		String yyyyMMdd = DayUtil.DATE_FOR_DB.get();
		return new Day(yyyyMMdd);
	}

	/** 일/월/년을 각각 더한다. */
	public void plus(Integer... time) {
		Calendar thisCalendar = DayUtil.getCalendar(returnValue().toString());
		Calendar added = DayUtil.addCalendar(thisCalendar, time);
		String yyyyMMdd = DayUtil.DATE_FOR_DB.get(added);
		initValue(yyyyMMdd);
	}
	...
	public void initValue(Object obj) {
		if(obj instanceof Map){
			...
			if(obj instanceof Date) obj = DayUtil.DATE_FOR_DB.get((Date)obj);
			String yyyyMMdd = StringUtil.getNumericStr(obj);
