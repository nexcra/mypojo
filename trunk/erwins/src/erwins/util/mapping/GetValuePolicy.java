...
/**
 * request 등의 자료 변환 타입을 정의한다. <br>
 * 1. 자식객체의 경우 name으로 property.~~형식을 사용한다. <br>
 * 2. 키값, Date를 제외한 컬렉션 포함 모든 객체는 null safe하다. <br>
 * 키값은 주로 부모 객체에만 존재하며 하이버네이트의 save or update 구분을 위해 사용된다.
 * @author erwins(my.pojo@gmail.com)
 */
public abstract class GetValuePolicy{
    ...
    protected boolean isMultiEntity = false;
    protected String preFixse = null;
    
    /**
     * 스트러츠 등에서 사용하는 방식이다. 기본은 false
     * 다중 entity선택을 가능하게 한다. 선택자는 클래스의 이름이다. (User => user)
     * @EX :  user.id && board.id 
     **/
    @SuppressWarnings("unchecked")
    @Deprecated
    public void isMulti(Class clazz){
        isMultiEntity = true;
        this.preFixse = clazz.getSimpleName() + "." ;
    }

    /**
     *  isMultiEntity 속성을 체크하여 preFixse여부를 결정한다.
     *  추후 교체
     */
    private String getKey(String key){
        return isMultiEntity ? preFixse + key : key;
    }
