...
import erwins.util.exception.runtime.LoginRequiredException;
import erwins.util.exception.runtime.RoleNotFoundException;
...
public abstract class ButtonTag<ID extends Serializable,ROLE> {
    ...
    public String make(Pair mode) {
        id = mode.getValue();
        return make(mode.getName());
    }
    
    /**
     * JSP에서 버튼을 만듭니다.
     **/
    public String make(String btnName) {
        try {
            return makeButton(btnName);
        }
     ...
     */
    private String makeButton(String btnName) {
        ...
        if (jsonEntity != null){
            ...
            btn.addAttribute("json",JSMapper.instance().getObject(jsonEntity));
        }
