...
/**
 * Ftp 등등을 래핑한다. 필요할때마다 로직을 추가하자. ex)1 .. ftpClient.changeWorkingDirectory("/public"); boolean result = ftpClient.makeDirectory("oops"); // /public/oops 절대경로를 적지 않아도 된다
...
public abstract class NetRoot {
    ...
    public void delete(String fullName) throws IOException {
    ...
    public void deleteDir(String fullName) throws IOException {
        boolean result = ftpClient.removeDirectory(fullName);
        if(!result) throw new IOException(fullName +" 를 지우는데 실패했습니다.");
        else directorydeleted++;
    }
    ...
    public void upload(File clientFile) throws IOException {
        ...
        String uploadName = getNameByClient(clientFile.getAbsolutePath());
        boolean result;
     ...
     */    
    protected String getNameByClient(String name) {
