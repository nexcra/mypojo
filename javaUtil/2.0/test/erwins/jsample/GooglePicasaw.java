...
public class GooglePicasaw {

    @Test
    public void test() throws IOException, ServiceException{
        PicasawebClient c = new PicasawebClient(new PicasawebService("영감님"),"mypojo@gmail.com","123");
        System.out.println(c.getAlbums().size());
        for(AlbumEntry entry : c.getAlbums()){
    ...
    private static class PicasawebClient {
        ...
        public String getLinkByRel(List<Link> links, String relValue) {
            for (Link link : links) {
                if (relValue.equals(link.getRel())) { return link.getHref(); }
            }
